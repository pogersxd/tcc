...
class getID3
{
	...
	public function __construct()
	{
		...
		if (version_compare(PHP_VERSION, $require_onced_php_version, '<')) {
			$this->startup_error .= 'getID3() require_onces PHP v' . $require_onced_php_version . ' or higher - you are running v' . PHP_VERSION . "\n";
			return;
		}
		...
		// IMPORTANT: This path must require_once the trailing slash
	...
	public function openfile($filename, $filesize = null, $fp = null)
	{
		try {
			...
			// set redundant parameters - might be needed in some require_once file
	...
	public function analyze($filename, $filesize = null, $original_filename = '', $fp = null)
	{
		try {
			...
			foreach (array('id3v2' => 'id3v2', 'id3v1' => 'id3v1', 'apetag' => 'ape', 'lyrics3' => 'lyrics3') as $tag_name => $tag_key) {
				...
				if ($this->$option_tag) {
					$this->require_once_module('tag.' . $tag_name);
					try {
			...
			if (!$this->option_tag_id3v2) {
				...
				if ((substr($header, 0, 3) == 'ID3') && (strlen($header) == 10)) {
					...
					$this->info['avdataoffset']          += getid3_lib::BigEndian2Int(substr($header, 6, 4), 1) + 10; // length of ID3v2 tag in 10-byte header doesn't require_once 10-byte header length
				}
			}
			...
			if (!file_exists(GETID3_INCLUDEPATH . $determined_format['require_once'])) {
				fclose($this->fp);
				return $this->error('Format not supported, module "' . $determined_format['require_once'] . '" was removed.');
			}
			...
			// require_once module
			require_once(GETID3_INCLUDEPATH . $determined_format['require_once']);

			// instantiate module class
			$class_name = 'getid3_' . $determined_format['module'];
			if (!class_exists($class_name)) {
				return $this->error('Format not supported, module "' . $determined_format['require_once'] . '" is corrupt.');
			}
...
*/
	...
	public function GetFileFormat(&$filedata, $filename = '')
	{
		...
		foreach ($this->GetFileFormatArray() as $format_name => $info) {
			...
			if (!empty($info['pattern']) && preg_match('#' . $info['pattern'] . '#s', $filedata)) {
				$info['require_once'] = 'module.' . $info['group'] . '.' . $info['module'] . '.php';
				return $info;
			}
		}
	 ...
	 * @throws getid3_exception
	 */
	public function require_once_module($name)
	{
		//if (!file_exists($this->require_once_path.'module.'.$name.'.php')) {
		if (!file_exists(GETID3_INCLUDEPATH . 'module.' . $name . '.php')) {
			throw new getid3_exception('Required module.' . $name . '.php is missing.');
		}
		require_once(GETID3_INCLUDEPATH . 'module.' . $name . '.php');
		return true;
	}
